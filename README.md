...
| `--temperature` | Sampling-temperature override applied consistently across all backends (vLLM / transformers / web API). **By default** each Q&A mode uses its own default: `0` for multiple-choice / numeric (deterministic token-probability reads), `1` for chain-of-thought / reasoning. Web APIs that reject `temperature` (e.g. OpenAI o1/o3) have it filtered out with a logged warning. | `0.0`, `0.7`, `1.0` |
...
options:
  ...
  --temperature TEMPERATURE
                        [float] Sampling temperature override. If unset, each Q&A mode uses its default (0.0 for multiple-choice/numeric, 1.0 for chain-of-thought / reasoning).
