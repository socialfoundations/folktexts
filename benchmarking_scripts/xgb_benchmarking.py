...
if __name__ == "__main__":
    datasets = ["acsincome_classification.csv", "acspubcov.csv", "acsunemployment_filtered.csv", "college_scorecard.csv", "ipums.csv", "sepsis.csv", "diabetic_data.csv"]
    label_cols = ["PINCP", "PUBCOV", "ESR", "Outcome", "facility", "SepsisLabel", "readmitted"]
    ...
    for dataset_name, label_col in zip(datasets, label_cols):
        ...
        if dataset_name == "acspubcov.csv": # for pubcov, 1 = yes, 2 = no; map to 1/0
            y_train = -1 * y_train + 2
        if dataset_name == "diabetic_data.csv":
            y_train = y_train.map(lambda x: 1 if x == "<30" else 0)
        if dataset_name == "ipums.csv":
        ...
        output_csv_path = "benchmark_metrics_new.csv"
